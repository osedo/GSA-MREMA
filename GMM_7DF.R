GMM_7DF <- function(sum_exp_raw_count, raw.gs, set_number = NULL){
  
  
  # filter out genes not in list of gene sets and lowly expressed genes
  igenes <- intersect(rownames(assay(sum_exp_raw_count)), unique(unlist(raw.gs)))
  if(!length(igenes)) stop("Expression dataset (se)", " and ", "gene sets (gs) have no gene IDs in common")
  se <- assay(sum_exp_raw_count)[igenes,]
  group <- colData(sum_exp_raw_count)$GROUP
  keep <- filterByExpr(se, group = group)
  se <- se[keep,]
  raw.gs <- lapply(raw.gs, function(s) s[s %in% rownames(se)]) 
  lens <- lengths(raw.gs)
  ...
  dea_raw_count<- DESeqDataSetFromMatrix(countData = se, colData = colData(sum_exp_raw_count), design = ~ GROUP)
  dds <- DESeq(dea_raw_count)
  res <- results(dds)
  postdata <- tibble("genesEnsembl" = rownames(res), "GROUP_effect" = res[,2], "GROUP_variance" = res[,3]^2)
...
GMM_1DF <- function(sum_exp_raw_count, raw.gs, set_number = NULL){
  ...
  p_adj <- p.adjust(v, method = "BH", n = length(v))
  detected_gene_sets<- tibble("GeneSets"=names(raw.gs),"p-values"=v, "mid_dist"=nonDE_criterion, "size"=tol, "BIC_value"=BIC, "Adj-Pval" = p_adj)
  ###result <- list("table" = detected_gene_sets, "DEA" = res)
  return(detected_gene_sets) 
