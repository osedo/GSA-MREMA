...
mrema<- function(sum_exp_raw_count, raw.gs){
 # the followings are the EM criterion
  ...
  m<- 1e-6
  iter<- 10
  # we follow DESeq2 workflow to normalize data, this is optional users can use and other 
  # packages. It should be noted that users could also use microarray data 
  dea_raw_count<- DESeqDataSetFromMatrix(countData = assay(sum_exp_raw_count), 
                                         colData = colData(sum_exp_raw_count),
                                         design = ~ GROUP)
