...
Group0_samples <- as.list(tumour_subtypes[which(tumour_subtypes[,row_number] == group_0_name),1])
Group1_samples <- as.list(tumour_subtypes[which( tumour_subtypes[,row_number] == group_1_name ),1])

Group0_samples <- as.character(unlist(Group0_samples))
Group1_samples <- as.character(unlist(Group1_samples))

# get same name format as subgroup data
if (nchar(colnames(data[,1])) > 12){
  colnames(data) <- substr(colnames(data), start = 1, stop = 12)
}

if (nchar(Group0_samples[1])>12){
Group0_samples <- substr(Group0_samples, start = 1, stop = 12)
Group1_samples <- substr(Group1_samples, start = 1, stop = 12)
}


# create two summarized experiment object group 0 and group 1
group_0_SEO <- subset(data, select = colnames(data) %in% Group0_samples)
group_1_SEO <- subset(data, select = colnames(data) %in% Group1_samples)
...
## remove genes with same externalgene id
gene_names <- rowData(data)$external_gene_name
dup_gene_name <- which(duplicated(gene_names) == TRUE)

if(length(dup_gene_name) > 0){
  combined_counts <- combined_counts[-dup_gene_name,]
  gene_names <- gene_names[-dup_gene_name]
}

## change rownames from ensemble ids to gene names
rownames(combined_counts) <- gene_names
...
## only keep counts for genes that are in our gene sets
combined_counts <- combined_counts[which(rownames(combined_counts) %in% msigdb_gene_sets$human_gene_symbol == TRUE),]

## remove genes from sets that are not in count data
genes_insets <- msigdb_gene_sets[-which((msigdb_gene_sets$human_gene_symbol %in% rownames(combined_counts)) == FALSE),]

which((genes_insets$human_gene_symbol %in% rownames(combined_counts)) == FALSE)
which((rownames(combined_counts) %in% genes_insets$human_gene_symbol) == FALSE)

keep_rownames <- rownames(combined_counts)
...
rownames(sum_exp_raw_count) <- keep_rownames
